					        ...
					        <table class="table table-bordered">
					            <thead style="background-color:#f3f3f3;">
					                <tr>
					                    ...
					                    <th>End Date</th>
										<th>Total days</th>
										<th class="admin">Requested By</th>
										<th>Status</th>
										<th>Action</th>
										<!-- <th>Action</th> -->
					                </tr>
					            ...
					            <tbody>
					            	...
					            	@if (count($leave_request) > 0)
						                ...
						                <tr>
											...
											<td class="admin">{{ $l->users->name }}</td>
											<td>{{ $l->status }}</td>
						                    <td>
												<a href="@if($l->status == 'approved') javascript:void(0); @else /leave-edit/{{ $l->id }} @endif" class="btn-medi medi-yellow @if($l->status == 'approved') not-allowed @endif" style="margin-right:5px;"> <i class="fas fa-edit"></i> </a>
												<a href="@if($l->status == 'approved') javascript:void(0); @else /leave-cancel/{{ $l->id }} @endif" class="btn-medi medi-red @if($l->status == 'approved') not-allowed @endif" style="margin-right:5px;"> Cancel </a>
					        ...
					        <table class="table table-bordered">
					            <thead style="background-color:#f3f3f3;">
					                <tr>
					                    ...
					                    <th>End Date</th>
					                    <th class="admin">Requested By</th>
					                    <th class="admin">Requested To</th>
										<th>status</th>
										<th>Action</th>
					                </tr>
					            </thead>
					            <tbody>
					            	...
					            	@if (count($substitute_leave) > 0)
						                ...
						                <tr>
											...
											<td><?php echo date('d F Y', strtotime($l->leave_dates->last()->requested_date)); ?></td>
											<td class="admin">{{ $l->users->name }}</td>
											<td class="admin">{{ $l->substitute->name }}</td>
											<td>{{ $l->status }}</td>
											<td>
												<a href="@if($l->status == 'approved') javascript:void(0); @else /leave-accept/{{ $l->id }} @endif" class="btn-medi medi-green @if($l->status == 'approved') not-allowed @endif" style="margin-right:5px;"> Accept </a>
											    <a href="@if($l->status == 'approved') javascript:void(0); @else /leave-approve/{{ $l->id }} @endif" class="btn-medi medi-green admin @if($l->status == 'approved') not-allowed @endif" style="margin-right:5px;"> Approve </a>
											    <br>
											    <div style="margin-top:20px;">
												    <a href="@if($l->status == 'approved') javascript:void(0); @else /leave-reject/{{ $l->id }} @endif" class="btn-medi medi-red @if($l->status == 'approved') not-allowed @endif" style="margin-right:5px;"> Reject </a>
